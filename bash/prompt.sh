...
function __prompt_command() {
    ...
    PS1+="\[${BOLD}${MAGENTA}\]\u \[$RESET\]/ \[$ORANGE\]\h \[$RESET\]/ \[$MAGENTA\]\w\[$RESET\]\$([[ -n \$(git branch 2> /dev/null) ]] && echo \" / \")\[$PURPLE\]\$(parse_git_branch)\n"

    if [ $EXIT != 0 ]; then
        PS1+="\[${MAGENTA}\][${EXIT}]\[${RESET}\] "      # Add red if exit code non 0
    fi
